...
public:
    ...
    void print_depth(std::function<void(T&, size_t)> print_, size_t depth) {
		Tree* c = LocateCurrentTree();
		if (!c) return;
        print_(c->data, depth);
        for (size_t i = 0; i < c->nodes.size(); i++) {
			c->nodes[i].print_depth(print_, depth + 1);
		}
	}
    void print(std::function<void(T&, size_t)> print_) {
        print_depth(print_, 0);
    }
};